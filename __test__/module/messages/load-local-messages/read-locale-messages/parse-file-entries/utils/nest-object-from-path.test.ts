...
import { nestObjectFromPath } from "@/modules/messages/load-local-messages/read-locale-messages/parse-file-entries/utils/nest-object-from-path";

describe("nestObjectFromPath", () => {
  it("should return value directly if path is empty", () => {
    const value: NamespaceMessages = { a: "A" };
    const result = nestObjectFromPath([], value);
    expect(result).toEqual({ a: "A" });
  });

  it("should wrap value with a single key", () => {
    const value: NamespaceMessages = { b: "B" };
  ...
  it("should nest value correctly for multiple path segments", () => {
    const value: NamespaceMessages = { d: "D" };
  ...
  it("should work with numbers or other strings as keys", () => {
    const value: NamespaceMessages = { x: "1" };
