import type {
  ...
  NamespaceGroupValue,
} from "../../../intor-messages-loader/load-local-messages/prepare-namespace-groups";
import fs from "node:fs/promises";
import path from "node:path";
import { addToNamespaceGroup } from "../../../intor-messages-loader/load-local-messages/prepare-namespace-groups";
...
export const traverseDirectory = async ({
  ...
  const { limit, logger: baseLogger } = options;
  const logger = baseLogger?.child({ prefix: "traverseDirectory" });
  ...
  try {
    ...
    const dirPromises = dirents.map((dirent) =>
      limit(async () => {
        ...
        void logger?.warn("Failed to process a locale file or directory.", {
          name: dirent.name,
    ...
    void logger?.warn(`Error reading directory: ${currentDirPath}`, { error });
  }
};
